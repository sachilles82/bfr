...
<div x-show="menu" @click.away="menu = false" @keydown.window.escape="menu = false" x-cloak
    ...
    <!-- Background Overlay -->
    <div x-show="menu" @click="menu = false" x-transition.opacity.duration.600ms
        ...
        <div class="flex flex-col h-full py-5">
            <div class="flex items-center justify-between px-4">
                <img class="h-8 w-auto" src="https://tailwindui.com/plus/img/logos/mark.svg?color=white"
                     alt="Your Company">
                <button @click="menu = false" class="text-white focus:outline-none">
            ...
            <div class="mt-5 h-0 flex-1 overflow-y-auto px-4">
                <nav class="space-y-1">
                    ...
                    <a href="{{ route('settings.roles') }}" @click="menu = false"
                       wire:navigate.hover
                       class="{{ request()->routeIs(['settings.roles', 'settings.departments', 'settings.departments.show']) ? 'text-white bg-indigo-800 dark:text-white dark:bg-gray-800' : 'dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800 text-indigo-100 hover:bg-indigo-800 hover:text-white' }} group flex items-center rounded-md px-3 py-2 text-sm font-medium">
                        <svg
                            class="{{ request()->routeIs(['settings.roles', 'settings.departments', 'settings.departments.show']) ? 'dark:text-white dark:bg-gray-800' : 'text-indigo-300 dark:text-gray-400 group-hover:text-white' }} mr-3 h-6 w-6"
                            fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round"
