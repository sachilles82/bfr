    ...
    <div class="flex w-28 flex-col items-center py-6">
        ...
        <div class="mt-6 w-full flex-1 space-y-1 px-2">
            ...
            <a href="{{ route('settings.roles') }}" wire:navigate.hover
               class="{{ request()->routeIs(['settings.roles', 'settings.departments']) ? 'text-white bg-indigo-800 dark:text-white dark:bg-gray-800' : 'dark:text-gray-400 dark:hover:text-white dark:hover:bg-gray-800 text-indigo-100 hover:bg-indigo-800 hover:text-white' }} group flex w-full flex-col items-center rounded-md p-3 text-xs font-medium">
                <svg
                    class="{{ request()->routeIs(['settings.roles', 'settings.departments']) ? 'dark:text-white dark:bg-gray-800' : 'text-indigo-300 dark:text-gray-400 group-hover:text-white' }} h-6 w-6"
                    fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                    <path stroke-linecap="round" stroke-linejoin="round"
