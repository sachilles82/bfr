    ...
    x-data="{
        ...
        init() {
            $(this.$refs.picker).daterangepicker({
                ...
                autoApply: true,
                locale: {
                    format: 'MM/DD/YYYY' // Anpassen nach Bedarf
                }
            },
            (date) => {
                this.value = date;
                this.$wire.set('filter.date', date.format('MM/DD/YYYY'));
            });
...
>
    <div class="w-full relative flex items-center gap-2 rounded-md pr-2 text-gray-600 text-sm">
        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3">
            <x-icon.calendar/>
        </div>
        <input type="text" x-ref="picker" readonly class="min-w-60 py-2 px-3 pl-12 block w-full text-sm font-medium
...
<!-- Date Range Picker ohne Ranges -->
{{--<div--}}
{{--    x-data="{--}}
{{--        start: moment().startOf('hour'),--}}
{{--        end: moment().startOf('hour').add(32, 'hour'),--}}
{{--        init() {--}}
{{--            $(this.$refs.timepicker).daterangepicker({--}}
{{--                timePicker: true,--}}
{{--                showDropdowns: true,--}}
...
{{--                this.start = start;--}}
{{--                this.end = end;--}}
{{--                this.$dispatch('input', { start: start.format('M/DD hh:mm A'), end: end.format('M/DD hh:mm A') });--}}
{{--            });--}}

{{--            this.$watch('start', (newValue) => {--}}
{{--                const picker = $(this.$refs.timepicker).data('daterangepicker');--}}
{{--                picker.setStartDate(newValue);--}}
{{--            });--}}

{{--            this.$watch('end', (newValue) => {--}}
{{--                const picker = $(this.$refs.timepicker).data('daterangepicker');--}}
{{--                picker.setEndDate(newValue);--}}
{{--            });--}}
{{--        },--}}
...
{{--    class="max-w-sm w-full"--}}
{{-->--}}
{{--    <div class="w-full relative flex items-center gap-2 rounded-md text-gray-600 text-sm justify-center w-full md:w-auto">--}}
{{--        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-2">--}}
{{--            <x-icon.calendar/>--}}
{{--        </div>--}}
{{--        <input type="text" x-ref="picker" readonly class="md:min-w-56 py-2.5 px-3 pl-10 block w-full text-sm font-medium--}}
