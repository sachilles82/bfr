...
<button type="button" {{ $attributes->class($classes) }} data-flux-profile>
    ...
    <?php if ($name): ?>
        <span class="ml-2 text-sm text-zinc-500 dark:text-white/80 group-hover:text-zinc-800 group-hover:dark:text-white font-medium leading-none truncate">
            {{ $name }}
        </span>
    <?php endif; ?>

    <?php if ($chevron): ?>
        <div class="ml-auto size-8 flex justify-center items-center">
